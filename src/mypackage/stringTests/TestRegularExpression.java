...
public class TestRegularExpression {
	...
	public static void main(String[] args) {
		 ...
		 * "charactorSwquence regularExpression+"); System.exit(0); } System.out.println("Input: \""
		 * + args[0] + "\""); for (String arg : args) {
		 * System.out.println("Regullar expression : \"" + arg + "\""); Pattern p =
		 * Pattern.compile(arg); Matcher m = p.matcher(args[0]); while (m.find()) {
		 * System.out.println("Match \"" + m.group() + "\" atposition " + m.start() + "-" +
		 * (m.end()-1)); } }
		 */
	...
	static class TestScanner {
		...
		TestScanner(String str) {
			TestScanner.sc = new Scanner(new BufferedReader(new StringReader(
					str)));
		...
		public void display() {
			System.out.println("int: " + this.intValue + " long: "
					+ this.longValue + " float: " + this.floatValue
					+ " double:" + this.doubeValue + " string:"
					+ this.stringValue);
		}
	}
	...
	public static void practice6() {
		...
		File file = new File(
				"E:\\code\\ecli\\dostring\\src\\regularExpression\\TestRegularExpression.java");
		Scanner sc = null;
		try {
			sc = new Scanner(file);
		...
		for (int i = 0; i < array.length; i++) {
			array[i] += "*/";
			m = Pattern.compile("/\\*(.*)\\*/", Pattern.DOTALL).matcher(
					array[i]);
			if (m.find()) {
				System.out.println(m.group());
			}
	...
	public static void practice1() {
		...
		String[] patterns = { "^Java", "\\Breg", "n.w\\s+h(a|i)s", "s?", "s*",
				"s+", "s{4}", "s{1}.", "s{0,3}", "^[A-Z].*\\.$" };
